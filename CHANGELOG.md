...
- [`sw.js`](sw.js) now caches [`js/otc-explorer.js`](js/otc-explorer.js) and bumps `STATIC_CACHE_VERSION` to `rod-wallet-static-v2.8.0-alpha.0`.
...
- Harness isolation now matches the e2e runner for same-origin probe checks: [`tests/harness/unit-browser-test.js`](tests/harness/unit-browser-test.js) creates its Playwright probe pages from a context with `serviceWorkers: 'block'`, preventing [`sw.js`](sw.js) from intercepting `/probe-tip/...` and `/probe-bchair/...` requests during explorer-adapter tests and restoring mock-route control over the final Blockchair JSON probe.
- Release metadata is now aligned for the alpha minor release: [`manifest.webmanifest`](manifest.webmanifest) version strings now match [`sw.js`](sw.js) cache key `rod-wallet-static-v2.8.0-alpha.0`, and [`sha1sum`](sha1sum) carries the `Version 2.8.0-alpha.0 2026.07.27` header with refreshed checksums for the edited release and harness files.
- Harness portability follow-up is now documented in-tree: [`tests/harness/run-all.sh`](tests/harness/run-all.sh) resolves Node through `node`, `node.exe`, or `NODE_BIN`, and both [`tests/harness/unit-browser-test.js`](tests/harness/unit-browser-test.js) and [`tests/harness/e2e-swap-test.js`](tests/harness/e2e-swap-test.js) accept `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH` or `PW_CHROMIUM_EXECUTABLE_PATH` while preserving the existing `/opt/pw-browsers/chromium` fallback.
- Final local proof-harness verification is now green on the previously blocked host: from [`tests/harness`](tests/harness), `npm exec playwright install chromium` plus `set DEBUG=pw:install&& node node_modules\playwright\cli.js install chromium` installed the matching Playwright `1.61.1` browser revision `1228` at [`C:/Users/VSCode/AppData/Local/ms-playwright/chromium-1228/chrome-win64/chrome.exe`](C:/Users/VSCode/AppData/Local/ms-playwright/chromium-1228/chrome-win64/chrome.exe) and [`C:/Users/VSCode/AppData/Local/ms-playwright/chromium_headless_shell-1228/chrome-headless-shell-win64/chrome-headless-shell.exe`](C:/Users/VSCode/AppData/Local/ms-playwright/chromium_headless_shell-1228/chrome-headless-shell-win64/chrome-headless-shell.exe), matching [`tests/harness/package-lock.json`](tests/harness/package-lock.json) and [`tests/harness/node_modules/playwright-core/browsers.json`](tests/harness/node_modules/playwright-core/browsers.json).
- Final proof evidence is now recorded: [`node unit-browser-test.js`](tests/harness/unit-browser-test.js) from [`tests/harness`](tests/harness) completed with `ALL UNIT CHECKS PASSED`, and [`bash tests/harness/run-all.sh`](tests/harness/run-all.sh) from the workspace root completed with `ALL SUITES PASSED` across unit checks plus LTC/DOGE/BTC/BCH happy, ROD-refund, alt-refund, and reload scenarios.

### Documentation
- Carbon Memory follow-up was refreshed after the release-metadata, harness-isolation, repository-understanding, and final proof-harness verification pass: [`.kilocode/rules/memory-bank/context.md`](.kilocode/rules/memory-bank/context.md) and [`.kilocode/rules/memory-bank/active.md`](.kilocode/rules/memory-bank/active.md) now record the completed release-metadata alignment, the resolved service-worker probe interception issue, the green proof harness, and the restored Understand Anything import-edge coverage.
- Repository-understanding follow-up is now closed as a documentation item: [`.understand-anything/config.json`](.understand-anything/config.json) still has `autoUpdate: true`, but no active host hook was proven because there is no [`.githooks`](.githooks) directory and no `core.hooksPath` entry in [`.git/config`](.git/config). A manual refresh via [`.understand-anything/tmp-understand-refresh.ps1`](.understand-anything/tmp-understand-refresh.ps1) regenerated the graph artifacts with `scan-project: filesScanned=52` and restored `3` import edges in [`.understand-anything/knowledge-graph.json`](.understand-anything/knowledge-graph.json), [`.understand-anything/meta.json`](.understand-anything/meta.json), [`.understand-anything/intermediate/review.json`](.understand-anything/intermediate/review.json), and [`.understand-anything/tmp/import-map.json`](.understand-anything/tmp/import-map.json).
