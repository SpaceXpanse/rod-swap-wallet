...
- Release metadata follow-up is now complete: [`manifest.webmanifest`](manifest.webmanifest) version strings were updated to `2.7.1-alpha.0` so they match the [`sw.js`](sw.js) cache key `rod-wallet-static-v2.7.1-alpha.0`, and [`sha1sum`](sha1sum) now carries the `Version 2.7.1-alpha.0 2026.07.27` header with refreshed checksums for the edited release and harness files.
