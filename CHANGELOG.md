...
### Documentation
- Updated public and maintainer documentation to match the current shipped OTC surface: support remains **ROD ↔ LTC** and **ROD ↔ DOGE** only, release verification now centers on [`tests/run-fast.sh`](tests/run-fast.sh), [`tests/harness/run-all.sh`](tests/harness/run-all.sh), and [`SHA256SUMS`](SHA256SUMS), and durable Carbon Memory pages now reflect the active browser/runtime architecture instead of the older divergence notes.
- Finalized release metadata for `2.9.0-beta.1`: [`manifest.webmanifest`](manifest.webmanifest) and [`sw.js`](sw.js) now carry the shipped version identity, the obsolete [`sha1sum`](sha1sum) manifest has been removed, and the release inventory is tracked only through [`SHA256SUMS`](SHA256SUMS).
...
- **Order detail travels over Nostr, anchored by the ROD name record.** The ROD name DB remains the **only** source of truth for what is on offer: an order exists because a name record exists, and nothing a relay says can add one to the book. The record names a Nostr event id and signing key, and the fuller payload is published as a signed event on the relay pool the swap protocol already maintains, which keeps the size-limited on-chain value from having to carry everything. The anchor is what makes this safe: detail is accepted only when the event id matches the id named on chain **and** its signature verifies under the key named on chain, so a relay can withhold detail but can never substitute, alter or invent an order. **The tradable terms are not detail** — price, size, side, pair, counter chain and the settlement window are read from the record only, so an anchor-only record is refused rather than silently taking its economics from a relay. Detail fills ancillary gaps and never overwrites a field the record states. Order events use kind `31340` in the NIP-01 parameterised-replaceable range so a re-publish supersedes cleanly under the same `d`-tag; superseding an *order*, though, means writing a new name record.
...
- **Wallet-only networks could enter the OTC flow.** The counter-chain dropdown was populated from the wallet network table, allowing BTC, BCH, and DGB to be selected even though this release's settlement engine implements only LTC and DOGE. The UI now derives its choices from the intersection of swap fees, chain definitions, and wallet networks; release and browser gates enforce that boundary.
...
- **Incoming refund deadlines were trusted without enforcing cross-chain ordering.** Both lock heights are now checked against fresh ROD and LTC/DOGE tips in wall-clock seconds before an incoming session is created, before local acceptance, before refund signing and immediately before either peer funds. ROD must remain refundable later than the counter leg by at least 30 minutes (or the larger configured confirmation window); unsafe terms hard-block signing and funding.
...
### Changed
- **Protocol roles are named for their economics: `alice` → `seller`, `bob` → `buyer`.** The seller sells ROD, funds the ROD leg and holds the adaptor secret; the buyer buys ROD with the counter asset. States `ALICE_ROD_FUNDED` and `BOB_ALT_FUNDED` become `SELLER_ROD_FUNDED` and `BUYER_ALT_FUNDED`, and terms fields `aliceChildPubKey` / `bobChildPubKey` become `sellerChildPubKey` / `buyerChildPubKey` — matching the `seller*` / `buyer*` naming the terms already used. This changes the canonical terms hash, so **both peers must run the same version**; mixed versions are rejected at the terms-hash check rather than failing later. Sessions persisted under the old vocabulary are migrated on read in both the engine and the storage module, which keeps legacy swaps visible **and refundable** — an un-migrated role would otherwise fall through every role branch and silence the refund monitor.
...
- **Unit regressions** for the legacy role migration (role, state, terms keys, timeline, idempotence) and for the adaptor invariant that motivates pinning: a secret recovers only under the point it was encrypted to, and a swapped point is rejected.
